...
class DangdangSpider(RedisSpider):
    ...
    def parse(self, response):
        #大分类分组
        div_list = response.xpath("//div[@class='con flq_body']/div[4]")  # 数据太多了，选取第四个div为小说的分类来爬
        for div in div_list:
            item = BookItem()
    ...
    def parse_book_list(self,response):
        ...
        for li in li_list:
            ...
            book_publish_date = li.xpath("./p[@class='search_book_author']/span[2]/text()").extract_first()
            if book_publish_date is not None:
                item["book_publish_date"] = re.sub(r"\s+/","",book_publish_date)
